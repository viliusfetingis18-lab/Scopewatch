...
## Execution Rules (apply to every function call)

- When a function is called (`/audit`, `/clause`, `/recover`, `/upsell`, `/bench`, `/dashboard`), return the output matching that function's schema. Do not add unsolicited commentary, critique of the user's inputs, or suggestions for how the skill itself should be improved, unless the user explicitly asks for that.
- If a required input field is missing, respond with a single, direct line naming the missing field and ask for it. Do not proceed by guessing or estimating on the user's behalf, and do not pad the response with an explanation of why guessing would be wrong.
- Never generate a number (hours, days, cost, revenue, headcount) that was not provided by the user or derivable by arithmetic from numbers the user provided. If a calculation requires an input the user hasn't given, stop and ask for it.
- Do not invent additional data points (extra pending requests, extra team members, extra SOW clauses) to make an output look more complete than the inputs support.
...
```json
{
  ...
  "properties": {
    ...
    "effort_estimate_hours": {
      "type": "object",
      "description": "A real effort estimate broken down by role, sourced from an engineer, historical benchmark, or comparable past delivery. This must come from the user, not be generated by the skill.",
      "properties": {
    ...
    "estimate_source": {
      "type": "string",
      "description": "Where the effort estimate came from (e.g., 'engineering lead sign-off', 'comparable Q1 rollout', 'PM judgment call - unverified')"
    },
    "hourly_rate_matrix": {
      "type": "object",
  ...
  "required": ["scope_item", "effort_estimate_hours", "estimate_source", "hourly_rate_matrix", "project_margin_target"]
}
```

**If `effort_estimate_hours` is not provided, do not run the calculation.** Respond only with: "I need an effort estimate (hours per role) before I can calculate exposure. Who's providing this, an engineer, a comparable past project, or a placeholder for demo purposes?" Do not generate a plausible-sounding number to fill the gap.
...
```json
{
  ...
  "estimate_source": "Engineering lead sign-off",
  "hourly_rate_matrix": {
    "senior_engineer": 350,
    "devops_engineer": 300,
    "qa_engineer": 200
...
**Only summarize the pending requests actually provided in `pending_scope_requests`.** If the array contains one item, the dashboard should reflect exactly one item, don't add fictional additional requests to make the summary look like a fuller portfolio view. If the user wants a multi-item demo, they need to explicitly provide multiple items (real or clearly-labeled synthetic ones), not have the skill generate them.
...
- If an effort estimate is missing from `/recover`, do not generate one. Ask for it and stop.
- If `/dashboard` is given fewer pending requests than would make a compelling summary, report on exactly what was given. Do not add invented items.
- This applies across every function: a missing required input is a stop-and-ask condition, not a prompt to estimate plausibly.
...
- **v1.1.0** (2026-07-15): Added global execution rules to stop unsolicited commentary; added required `effort_estimate_hours` and `estimate_source` fields to `/recover` to prevent fabricated hour estimates; added anti-fabrication guardrail to `/dashboard` so it only reports on requests actually provided. Fixed after adversarial testing revealed `/recover` would generate a plausible-sounding but unfounded effort estimate when none was supplied, and that estimate would silently propagate through `/bench` and `/upsell` into a client-facing draft.
